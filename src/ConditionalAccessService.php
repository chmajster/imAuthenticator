...
final class ConditionalAccessService
{
    ...
    public function evaluate(int $userId, array|int $app, array $context = []): array
    {
        ...
        $max=(int)($app['max_concurrent_sessions']??0);if($max>0){$row=$this->db->one('SELECT COUNT(*) AS c FROM oidc_sessions WHERE application_id=? AND user_id=? AND revoked_at IS NULL AND (expires_at IS NULL OR expires_at>NOW())',[(int)$app['id'],$userId]);$count=(int)($row['c']??0);$existingId=(int)($context['existing_session_id']??0);$existingActive=$existingId>0&&$this->db->one('SELECT 1 FROM oidc_sessions WHERE id=? AND application_id=? AND user_id=? AND revoked_at IS NULL AND (expires_at IS NULL OR expires_at>NOW())',[$existingId,(int)$app['id'],$userId]);if(($existingActive&&$count>$max)||(!$existingActive&&$count>=$max))return$this->deny('concurrent_session_limit');}
