...
final class ApplicationAccessService
{
    ...
    {
        return array_values(array_unique(array_column($this->matchingDynamicRules($userId, $applicationId, $context), 'effect')));
    }
    ...
    {
        $rules = $this->db->all('SELECT id,rule_type,effect,condition_json FROM dynamic_access_rules WHERE application_id=? AND enabled=1 ORDER BY priority,id', [$applicationId]);
        $matched = [];
        foreach ($rules as $rule) {
            $condition = json_decode((string)$rule['condition_json'], true);
            if (is_array($condition) && $this->matchesCondition($userId, (string)$rule['rule_type'], $condition, $context)) $matched[] = $rule;
        }
        return $matched;
    }
    ...
    private function matchesAttribute(int $userId, array $condition): bool
    {
        ...
        if ($key === '') return false;
        $row = $this->db->one('SELECT attribute_value FROM user_attributes WHERE user_id=? AND attribute_key=?', [$userId,$key]);
        $operator = (string)($condition['operator'] ?? 'equals');
        if ($operator === 'exists') return $row !== null;
        if (!$row) return false;
        $actual = (string)$row['attribute_value'];
        $expected = $condition['value'] ?? null;
        return match ($operator) {
            'equals' => $actual === (string)$expected,
            'not_equals' => $actual !== (string)$expected,
            'contains' => str_contains(mb_strtolower($actual), mb_strtolower((string)$expected)),
            'in' => is_array($expected) && in_array($actual, array_map('strval', $expected), true),
            default => false,
        };
    }

    private function matchesIp(string $ip, array $condition): bool
    {
        if ($ip === '') return false;
        $cidrs = $condition['cidrs'] ?? ($condition['cidr'] ?? []);
        if (is_string($cidrs)) $cidrs = [$cidrs];
        if (!is_array($cidrs)) return false;
        foreach ($cidrs as $cidr) if (Security::ipInCidr($ip, (string)$cidr)) return true;
        return false;
    }

    private function matchesCountry(string $country, array $condition): bool
    {
        $countries = $condition['countries'] ?? [];
        return $country !== '' && is_array($countries) && in_array(strtoupper($country), array_map(static fn($v) => strtoupper((string)$v), $countries), true);
    }
    ...
    private function matchesTime(array $condition): bool
    {
        ...
        $start = (string)($condition['start'] ?? '00:00');
        $end = (string)($condition['end'] ?? '23:59');
        $current = $now->format('H:i');
        return $start <= $end ? ($current >= $start && $current <= $end) : ($current >= $start || $current <= $end);
    }

    private function matchesCompound(int $userId, array $condition, array $context): bool
    {
