...
final class AuditIntegrityService
{
    ...
    public function verify(): array
    {
        ...
        foreach ($rows as $row) {
            if (($row['previous_hash'] ?: null) !== $expectedPrevious) return ['valid'=>false,'broken_at'=>(int)$row['id'],'reason'=>'previous_hash_mismatch'];
            $payload = json_encode([
            ...
            if (!hash_equals((string)$row['entry_hash'], $hash)) return ['valid'=>false,'broken_at'=>(int)$row['id'],'reason'=>'entry_hash_mismatch'];
            $expectedPrevious = (string)$row['entry_hash'];
        }
        return ['valid'=>true,'entries'=>count($rows),'head'=>$expectedPrevious];
    }
}
