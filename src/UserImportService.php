...
final class UserImportService
{
    ...
    public function importRows(array $rows, string $fileName, string $format, int $actorUserId, string $duplicateMode = 'skip'): array
    {
        ...
        try {
            foreach ($rows as $index=>$raw) {
                ...
                try {
                    ...
                    if($existing&&$duplicateMode==='skip'){
                        $summary['duplicate']++;$this->finishRow($importRowId,'duplicate',(int)$existing['id'],'Existing user matched by e-mail or username');continue;
                    }
                    if($existing){
                        $userId=(int)$existing['id'];$this->updateUser($userId,$row);$summary['updated']++;$status='updated';
                    }else{
                        $userId=$this->createUser($row);$summary['created']++;$status='created';
                    }
                    $warnings=$this->applyMembershipsAndAttributes($userId,$row);
                    $this->finishRow($importRowId,$status,$userId,$warnings===[]?null:implode('; ',$warnings));
                } catch (\Throwable $e) {
                    $summary['failed']++;$this->finishRow($importRowId,'failed',null,substr($e->getMessage(),0,500));
                }
            }
            ...
            $this->audit->write('users.import.failed','failure',$actorUserId,null,null,substr($e->getMessage(),0,500),['job_id'=>$jobId]);
            throw $e;
        }
    }
    ...
    {
        if($row['username']!==''){
            $collision=$this->db->one('SELECT id FROM users WHERE LOWER(username)=LOWER(?) AND id<>?',[$row['username'],$userId]);
            if($collision)throw new RuntimeException('username_collision');
        }
    ...
    private function findExisting(string $email,string $username): ?array
    {
        if($username!=='')return $this->db->one('SELECT id FROM users WHERE LOWER(email)=LOWER(?) OR LOWER(username)=LOWER(?) LIMIT 1',[$email,$username]);
        return $this->db->one('SELECT id FROM users WHERE LOWER(email)=LOWER(?) LIMIT 1',[$email]);
    }
    ...
    private function normalizeRow(array $row): array
    {
        ...
        $bool=static function(string $value):bool{return !in_array(strtolower(trim($value)),['0','false','no','nie','disabled','inactive'],true);};
        ...
        return [
            'name'=>$pick(['name','nazwa','displayname','display_name']),
            'email'=>strtolower($pick(['email','e-mail','mail'])),
            'username'=>$pick(['username','login','user_name','uzytkownik']),
            'enabled'=>$bool($pick(['enabled','active','aktywny','status'])?:'true'),
            'starts_at'=>$this->dateOrNull($pick(['account_starts_at','starts_at','start_date','data_rozpoczecia'])),
            'ends_at'=>$this->dateOrNull($pick(['account_ends_at','ends_at','end_date','data_zakonczenia'])),
            'groups'=>$this->splitList($pick(['groups','grupy'])),
            'roles'=>$this->splitList($pick(['roles','role','role_systemowe'])),
            'attributes'=>$attributes,
        ];
    }

    private function normalizeHeader(string $header): string{$header=preg_replace('/^\xEF\xBB\xBF/','',$header)??$header;$header=mb_strtolower(trim($header));$header=str_replace([' ','-'],['_','_'],$header);return$header;}
    ...
    private function dateOrNull(string $value): ?string{if($value==='')return null;$ts=strtotime($value);if($ts===false)throw new RuntimeException('invalid_date_'.$value);return date('Y-m-d H:i:s',$ts);}
