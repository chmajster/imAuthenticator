...
$mfa=new MfaService($db,$crypto,$audit,$config);
...
$scimOutbound=new ScimOutboundService($db,$crypto,$access,$audit);
...
return compact('config','configFile','db','audit','auditIntegrity','events','settings','mail','crypto','delivery','apiKeys','access','appAdmins','conditional','auth','mfa','risk','requests','reviews','sessions','clientSecrets','passkeys','scim','scimOutbound','directory','jwt','oauthProofs','signingKeys','logoutPropagation','oauthAdvanced','oauthSecurity','oidc','externalIdentity','saml','magicLinks','consents','requiredActions','emails','impersonation','launcher','toolkit','housekeeping');
