...
if (strtoupper($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
    ...
    if ($userId > 0) {
        ...
        if ($action === 'revoke') $access->revokeUser($appId, $userId, (int)$admin['id']);
        if (in_array($action, ['assign_role','remove_role'], true)) {
...
foreach ($users as $user) {
    ...
    $rows .= '<tr><td><a href="/admin/users/'.$uid.'/applications"><strong>'.Web::e($user['name']).'</strong></a></td><td>'.Web::e($user['email']).'</td><td>'.((bool)$user['enabled']?'Aktywny':'Wyłączony').'</td><td><span class="badge '.($has?'ok':'muted').'">'.($has?'Tak':'Nie').'</span><div class="muted">'.Web::e($direct).'</div></td><td>'.Web::e(implode(', ', $userRoles) ?: '—').'</td><td>'.Web::e($user['created_at'] ?: '—').'</td><td>'.Web::e($user['creator_email'] ?: '—').'</td><td><div class="actions compact">';
    $rows .= '<form method="post" class="inline"><input type="hidden" name="_csrf" value="'.Web::e(Security::csrfToken()).'"><input type="hidden" name="user_id" value="'.$uid.'"><input type="hidden" name="action" value="'.($has?'revoke':'grant').'"><button class="'.($has?'':'primary').'">'.($has?'Odbierz dostęp':'Nadaj dostęp').'</button></form>';
    if ($roles !== []) {
...
if ($rows === '') $rows = '<tr><td colspan="8" class="empty">Brak użytkowników spełniających kryteria.</td></tr>';
...
$content .= '<section class="card"><h2>Dodaj użytkownika</h2><form method="get" action="/admin/applications/'.$appId.'/users" class="toolbar"><input type="hidden" name="id" value="'.$appId.'"><input name="q" value="'.Web::e($q).'" placeholder="Szukaj po nazwie lub e-mailu"><button class="primary" type="submit">Szukaj</button>'.($q!==''?'<a class="button" href="/admin/applications/'.$appId.'/users">Wyczyść</a>':'').'</form></section>';
$content .= '<div class="table-wrap"><table><thead><tr><th>Użytkownik</th><th>E-mail</th><th>Status</th><th>Dostęp</th><th>Role w aplikacji</th><th>Data nadania</th><th>Kto nadał</th><th>Akcje</th></tr></thead><tbody>'.$rows.'</tbody></table></div>';
