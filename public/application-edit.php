...
if (!$app) { http_response_code(404); Web::page('Nie znaleziono', '<div class="alert danger">Nie znaleziono aplikacji.</div>', $user); }
if(!$appAdmins->canManage((int)$user['id'],$id,'manage_config')){http_response_code(403);Web::page('Brak dostępu','<div class="alert danger">Nie masz uprawnień do konfiguracji tej aplikacji.</div>',$user);}
...
if (strtoupper($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
    ...
    if ($errors === []) {
        $before=$db->one('SELECT name,description,url,icon,logout_url,frontchannel_logout_uri,backchannel_logout_uri,backchannel_logout_session_required FROM applications WHERE id=?',[$id]);
        ...
        $after=$db->one('SELECT name,description,url,icon,logout_url,frontchannel_logout_uri,backchannel_logout_uri,backchannel_logout_session_required FROM applications WHERE id=?',[$id]);
        $audit->write('application.configuration.updated', 'success', (int)$user['id'], null, $id, null, ['before'=>$before,'after'=>$after,'redirect_count'=>count($redirects)]);
...
$content .= '<form method="post" class="card"><input type="hidden" name="_csrf" value="'.Web::e(Security::csrfToken()).'"><label>Nazwa aplikacji<input name="name" value="'.Web::e($_POST['name'] ?? $app['name']).'" required></label><label>Opis<textarea name="description" rows="3">'.Web::e($_POST['description'] ?? $app['description'] ?? '').'</textarea></label><label>Adres aplikacji<input type="url" name="url" value="'.Web::e($_POST['url'] ?? $app['url']).'" required></label><label>Ikona/logo URL<input type="url" name="icon" value="'.Web::e($_POST['icon'] ?? $app['icon'] ?? '').'"></label><div class="form-row"><label>Typ aplikacji<input value="'.Web::e($app['app_type']).'" readonly></label><label>Typ integracji<input value="'.Web::e($app['integration_type']).'" readonly></label></div>';
...
$content .= '<h2>Logout</h2><label>Logout URL<input type="url" name="logout_url" value="'.Web::e($_POST['logout_url'] ?? $app['logout_url'] ?? '').'"></label><label>Front-channel logout URI<input type="url" name="frontchannel_logout_uri" value="'.Web::e($_POST['frontchannel_logout_uri'] ?? $app['frontchannel_logout_uri'] ?? '').'"></label><label>Back-channel logout URI<input type="url" name="backchannel_logout_uri" value="'.Web::e($_POST['backchannel_logout_uri'] ?? $app['backchannel_logout_uri'] ?? '').'"></label><label class="checkbox"><input type="checkbox" name="backchannel_logout_session_required" value="1" '.((isset($_POST['backchannel_logout_session_required'])||(!$_POST && (bool)($app['backchannel_logout_session_required']??1)))?'checked':'').'> Back-channel logout token ma zawierać sid</label><div class="alert warning">Zmiana Redirect URI i endpointów logout działa natychmiast. Wildcardy nie są akceptowane.</div><button class="primary" type="submit">Zapisz zmiany</button></form>';
Web::page('Edytuj aplikację', $content, $user);
